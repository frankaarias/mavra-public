    ...
    <nav className="global-nav">
      ...
      <div className="nav-right">
        <div className={`nav-links${mobileOpen ? ' open' : ''}`}>
          ...
          <a href="/#system" onClick={close}>System Map</a>
          <NavLink to="/brand" onClick={close}>Brand OS</NavLink>
          <a href="/#contact" onClick={close}>Contact</a>
        </div>
        <button className="theme-toggle" onClick={toggleTheme} aria-label="Cambiar tema" title={theme === 'light' ? 'Modo oscuro' : 'Modo claro'}>
          {theme === 'light' ? <MoonIcon /> : <SunIcon />}
        </button>
        <button className={`nav-toggle${mobileOpen ? ' open' : ''}`} onClick={() => setMobileOpen(open => !open)} aria-label="Menú">
          <span /><span /><span />
        </button>
      </div>
    </nav>
