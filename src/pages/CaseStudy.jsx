...
const COPY = {
 ...
 es: {
  ...
  decisions:[
   ...
   {title:'Usar la investigación para orientar el lanzamiento.',image:'/shots/research-en-20260917.jpg',alt:'Espacio de investigación en inglés con columnas de demanda, competencia y ajuste al producto',type:'Espacio de investigación · Captura en inglés',screen:true,facts:['Una lista de palabras clave no basta para saber si una búsqueda encaja con el producto.','Revisar juntas la demanda, la presencia de competidores y la relevancia para el producto antes de planificar publicidad.','Un espacio de investigación funcional, un plan de campañas y una lista de comprobación del lanzamiento. Documentan cómo elegir y organizar el trabajo; no acreditan un aumento de ventas.'],links:[['Abrir la investigación','/research'],['Examinar el plan de campañas','/campaigns'],['Ver el control de lanzamiento','/launch']]}
  ],
...
export default function CaseStudy(){
 ...
 <header className="editorial-header editorial-hero"><div><p className="case-eyebrow">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="editorial-lead">{copy.intro}</p><div className="case-hero-actions"><a className="case-button primary" href="#evidence">{copy.primary} <ArrowDown size={16}/></a><Link className="case-button secondary" to="/brand">{copy.secondary} <ArrowUpRight size={16}/></Link></div></div><figure><a href="/pinterest/col.jpg" target="_blank" rel="noreferrer"><img src="/pinterest/col.jpg" alt={copy.decisions[0].alt}/></a><figcaption>{copy.caption}</figcaption></figure></header>
 <section className="editorial-section"><p className="case-section-label">{copy.challenge}</p><h2>{copy.challengeTitle}</h2><p>{copy.challengeText}</p></section>
 <section className="editorial-section" id="evidence"><span id="system"/><p className="case-section-label">{copy.evidence}</p><h2>{copy.evidenceTitle}</h2>{copy.decisions.map(d=><article className="case-decision-story" key={d.title}><div><h3>{d.title}</h3><dl>{d.facts.map((f,i)=><div key={i}><dt>{copy.labels[i]}</dt><dd>{f}</dd></div>)}</dl><div className="editorial-links">{d.links.map(([l,to])=><Link key={to} to={to}>{l} →</Link>)}</div></div><figure><a href={d.image} target="_blank" rel="noreferrer" aria-label={(lang==='es'?'Ampliar: ':'Enlarge: ')+d.alt}><img className={'decision-image'+(d.screen?' decision-screenshot':'')} src={d.image} alt={d.alt} loading="lazy"/></a><figcaption>{d.type}</figcaption></figure></article>)}</section>
