...
export default function Creators() {
  ...
  return (
    <div style={S.page}>
      ...
      {/* HERO */}
      <div style={S.hero}>
        <span style={S.eyebrow}>{trText("For Creators & Collaborators")}</span>
        <h1 style={S.h1}>{trText("We're not looking")}<br />{trText("for promoters.")}</h1>
        <p style={S.tagline}>{trText("We're looking for people who already live inside this aesthetic.")}</p>
      </div>

      {/* THE SPACE */}
      <section style={S.section}>
        <span style={S.label}>{trText("The Brand")}</span>
        <h2 style={S.h2}>{trText("What ")}{trText(identity.name)}{trText(" Is")}</h2>
        <p style={S.body}>{trText(identity.name)}{trText(" makes objects that change how a room feels. A skull lamp that projects its shadow onto your wall. Resin skull sculptures that belong there year-round. Candle sets built for ritual, not decoration.")}</p>
        <p style={S.body}>{trText("These are objects that change the temperature of a room. Not figuratively.")}</p>
        <p style={S.body}>{trText("We're not seasonal. We're not Halloween. We're permanent.")}</p>
        <p style={S.body}>{trText("If your space — or the spaces you create content in — already speaks this language, we should talk.")}</p>
      </section>
      ...
      <section style={S.section}>
        <span style={S.label}>{trText("Who We're Talking To")}</span>
        <h2 style={S.h2}>{trText("We don't filter by follower count.")}<br />{trText("We filter by fit.")}</h2>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={S.card}>
            <span style={S.platformTag}>{trText("TikTok")}</span>
            <p style={S.body}>{trText("We're looking for creators who share dark home aesthetics, room transformations, day-in-my-life content, or product finds that fit a specific world. If your FYP is also your aesthetic mood board, we probably belong there.")}</p>
          </div>
          <div style={S.card}>
            <span style={S.platformTag}>{trText("Instagram")}</span>
            <p style={S.body}>{trText("Interior mood, curated spaces, dark academia, alt-home decor. If your grid or Reels feel lived-in and deliberate — not staged for an audience — this is for you.")}</p>
          </div>
          <div style={S.card}>
            <span style={S.platformTag}>{trText("YouTube")}</span>
            <p style={S.body}>{trText("Home tours, aesthetic vlogs, \"how I decorated my apartment\" content. Long-form lifestyle creators who bring their audience into their actual space.")}</p>
          </div>
        </div>

        <div style={{ ...S.card, marginTop: '8px' }}>
          <span style={S.notTag}>{trText("What we're NOT looking for")}</span>
          <p style={{ ...S.body, marginBottom: 0 }}>{trText("Paid promotional slots, scripted reviews, or \"I'm working with Brand X\" announcements. We want the product in your world — not a commercial in the middle of it.")}</p>
        </div>
      </section>
      ...
      {/* WHAT WE OFFER */}
      <section style={S.section}>
        <span style={S.label}>{trText("The Partnership")}</span>
        <h2 style={S.h2}>{trText("What We Offer")}</h2>

        <div style={S.grid2}>
          <div style={S.card}>
            <h3 style={S.h3}>{trText("Product, gifted.")}</h3>
            <p style={{ ...S.body, marginBottom: 0 }}>{trText("We send you one (or more) ")}{trText(identity.name)}{trText(" pieces — no return required, no obligation to post. We send it because we believe you'll want to.")}</p>
          </div>
          <div style={S.card}>
            <h3 style={S.h3}>{trText("Creative freedom, complete.")}</h3>
            <p style={{ ...S.body, marginBottom: 0 }}>{trText("No scripts. No mandatory captions. No watermarks. Your voice, your framing, your aesthetic.")}</p>
          </div>
          <div style={S.card}>
            <h3 style={S.h3}>{trText("Commission.")}</h3>
            <p style={{ ...S.body, marginBottom: 0 }}>{trText("If you're already on Amazon, we connect your storefront. If you're not — we walk you through it. Takes one afternoon and every piece of content you create keeps earning after it's posted.")}</p>
          </div>
          <div style={S.card}>
            <h3 style={S.h3}>{trText("Early access.")}</h3>
            <p style={{ ...S.body, marginBottom: 0 }}>{trText(identity.name)}{trText(" is early. The creators who come in now become the reference point for everyone who discovers the brand later. There's value in being first.")}</p>
          </div>
        </div>
      </section>
      ...
      {/* CONTENT */}
      <section style={S.section}>
        <span style={S.label}>{trText("The Content")}</span>
        <h2 style={S.h2}>{trText("What resonates with")}<br />{trText("the people who buy.")}</h2>
        <p style={S.body}>{trText("We're not going to give you a shot list. But here's what we've seen work:")}</p>
        {[
          ...
          <div key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', marginBottom: '16px' }}>
            ...
            <p style={{ ...S.body, marginBottom: 0 }}>{trText(item)}</p>
          </div>
        ))}
      </section>
      ...
      {/* WHAT TO EXPECT */}
      <section style={S.section}>
        <span style={S.label}>{trText("The Process")}</span>
        <h2 style={S.h2}>{trText("What you can expect")}<br />{trText("after you email us.")}</h2>
        ...
        {[
          ...
          <div key={s.n} style={S.step}>
            <span style={S.stepNum}>{trText(s.n)}</span>
            <p style={{ ...S.body, marginBottom: 0 }}>{trText(s.text)}</p>
          </div>
        ))}

        <div style={{ ...S.card, marginTop: '8px', borderColor: 'rgba(var(--copper-rgb),0.4)' }}>
          <span style={S.label}>{trText("FTC Disclosure")}</span>
          <p style={{ ...S.body, marginBottom: 0 }}>{trText("We'll brief you on disclosure requirements before you post. Simple, clear, no friction — you stay protected and the content stays authentic.")}</p>
        </div>
      </section>
      ...
      {/* PRODUCTS */}
      <section style={S.section}>
        <span style={S.label}>{trText("The Products")}</span>
        <h2 style={S.h2}>{trText("What you'd be working with.")}</h2>
        <div style={S.grid3}>
          {[
            ...
            <div key={p.name} style={S.productCard}>
              <p style={S.productName}>{trText(p.name)}</p>
              <p style={{ ...S.productLine, marginBottom: 0 }}>{"\""}{trText(p.line)}{"\""}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* FAQ */}
      <section style={S.section}>
        <span style={S.label}>{trText("Common Questions")}</span>
        <h2 style={S.h2}>{trText("FAQ")}</h2>
        {creators.faqs.map((f, i) => (
          <div key={i} style={S.faqItem}>
            <p style={S.faqQ}>{trText(f.q)}</p>
            <p style={{ ...S.faqA, marginBottom: 0 }}>{trText(f.a)}</p>
          </div>
        ))}
      </section>
      ...
      {/* CTA */}
      <div style={S.ctaBox}>
        <h2 style={S.ctaH2}>{trText("If your space already speaks this language, we should be in it.")}</h2>
        <p style={S.ctaBody}>{trText("Send us a quick email — one paragraph about who you are and what you create. Platform, aesthetic, why ")}{trText(identity.name)}{trText(" fits your world. That's all we need.")}</p>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          <div style={S.emailBlock}>
            <span style={S.emailLabel}>{trText("Email")}</span>
            <span style={S.emailVal}>{trText("info@mavra.space")}</span>
          </div>

          <div>
            <span style={S.emailLabel}>{trText("Subject line format")}</span>
            <div style={S.subjectFormat}>{trText(identity.name)}{trText(" Collab — [Platform] — [Your Handle]")}</div>
            <p style={{ fontSize: '0.8rem', color: VAR.muted, marginTop: '8px' }}>{trText("Example: ")}{trText(identity.name)}{trText(" Collab — TikTok — @yourusername")}</p>
          </div>

          <p style={{ fontSize: '0.82rem', color: VAR.muted, maxWidth: '420px', textAlign: 'center', marginTop: '8px' }}>{trText("The subject format helps us route your message and respond faster. We won't be able to assist with Amazon order-related questions via this channel.")}</p>
          ...
          >{trText("Start the conversation →")}</a>
        </div>
      </div>
