...
const INFO_I18N = {
  ...
  use_GK: `Backend, los términos que el comprador no ve: ${USAGE_AYUDA}`,
  use_IH: `Item Highlight, bajo el título: ${USAGE_AYUDA}`,
  // 🔴 EL FORMATO DE ESTAS AYUDAS ES UNA REGLA, NO UN GUSTO. Frank, 2026-09-20:
  // «Los hover estan muy mal presentados». Venian asi:
  //
  //     Vol — búsquedas mensuales del término. · dato de mercado
  //
  // Tres problemas en una linea: la abreviatura en vez del nombre, la raya
  // separando ideas —que su regla dura prohibe— y la fuente colgando de un `·`
  // en minuscula, como una nota al pie que no se lee. Asi queda:
  //
  //     Volumen: Búsquedas mensuales del término. Dato de mercado.
  //
  ...
  en: {
    ...
    use_GK: `Backend, the terms the shopper does not see: ${USAGE_AYUDA_EN}`,
    use_IH: `Item Highlight, under the title: ${USAGE_AYUDA_EN}`,
    // Mismo formato que el castellano y por el mismo motivo: nombre completo,
    // dos puntos, la frase y la fuente como frase aparte. Sin rayas, sin `·` y
    // sin segunda persona: el «your product» pasa a ser «the ASIN».
    kw: 'Keyword: the term exactly as a shopper types it on Amazon. Market data.',
    root: 'Root: groups a keyword family. AGTA calculation.',
    vol: 'Volume: monthly searches for the term. Market data.',
    sales: 'Sales: units the market sells through that term. It is the demand that turns into a purchase. Market data.',
    rel: 'Relevancy: what share of the competitors sits on page one for that term. AGTA calculation.',
    p1: 'Page one: how many of the competitors are on page one for that term. AGTA calculation.',
    fit: 'Fit: how much the keyword belongs to the ASIN. Not the same as relevancy: relevancy measures how much the competitors own it, fit measures whether it is any use to the ASIN. AGTA calculation.',
    idn: 'Capturable demand: volume times fit. Ranks keywords by what the ASIN can take, not by what gets searched. AGTA calculation.',
    td: 'Titles: how many of the top listings use the keyword in their title. Market data.',
    cp: 'Competition: how many products compete for that term. Market data.',
    // `tier`, `prio` y `match` se fueron con sus columnas, igual que en el
    // castellano: la tabla de hoy no las tiene.
    cuando: 'When to attack it, UKL only. NOW: the ASIN satisfies it and the gap is open. LATER: the gap is closed or it is seasonal. NO: do not bid. AGTA calculation.',
    para: 'What it is good for, UKL only. SELL: the ASIN satisfies that search. TARGET: a sibling does, so target its page instead of bidding. CATALOG: niche demand not made today. AUDIENCE: tells who the buyer is, not what to sell. AGTA calculation.',
    serp: 'Results: what shows up on the search page. SBV is Sponsored Brand Video, AC is Amazon’s Choice and SP is Sponsored Product. Filter by typing SBV, AC or SP.',
    root_root: 'Root: a word or phrase that repeats across the core. If it shows up in a single keyword it is not a root. Each root is a PPC campaign, attacked one at a time. AGTA calculation.',
    root_frec: 'Frequency: how many MKL keywords this root appears in. It recalculates when keywords move between buckets.',
    root_sv: 'Broad volume: the total volume of every MKL keyword containing the root. It is the traffic ceiling of the family, not what gets captured.',
    root_kws: 'The MKL keywords containing any of the checked roots. Adding roots widens the coverage.',
    ...
    norm_sv: 'Volume: the summed volume of every keyword that collapsed into this form. It is the real demand for the idea, not for one way of writing it.',
    norm_n: 'Variants: how many MKL keywords collapsed into this form. They are ways of writing the same thing, and in PPC they bid against each other and push the CPC up, so only one is run.',
    ...
    compra_mil: 'Keyword conversion: what share of that term’s searches ends in a purchase. It belongs to the market, not to the ASIN, and it reads against the niche median. AGTA calculation.',
    trend: 'Trend: how the volume moves. Above +80% it is a seasonal keyword, so for a year-round product that volume does not count however large it looks.',
    price_fit: 'Price: median per-unit price of those ranking there, against the ASIN. Below 60% they sell far cheaper; above 160% they charge more.',
    veredicto: 'Verdict: the signal in one word, among ATTACK, PRICE ABOVE, PRICE BELOW, LOW CVR and SEASONAL. Hover the value to see the number behind it.',
  },
}
